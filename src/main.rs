		...
		client.with_framework(
			StandardFramework::new()
				...
				.on("stats", |ctx, msg, mut args| {
					fn parse(args: &mut serenity::framework::standard::Args, msg: &Message)
...
fn create_info_embed(
	...
	if let Some(next) = xp_user.left(&ranks).get(0) {
		if let Some(current) = xp_user.level(&ranks) {
			...
			serenity::builder::CreateMessage::default().embed(|mut e: serenity::builder::CreateEmbed| { e = e.author(|a| a.name("Blast — Statistics").icon_url(BLAST_ICON_URL)).description(format!("{} at rank <@&{}> with **{:.3}** XP. {} need **{:.3}** more XP to advance to rank <@&{}>.", {
				if myself {
					"You're currently".to_string()
				} else {
					format!("<@!{}> is", xp_user.user_id.0.to_string())
				}
			}, current.role_id.0.to_string(), xp_user.meta.xp, {
				if myself {
					"You"
				} else {
					"They"
